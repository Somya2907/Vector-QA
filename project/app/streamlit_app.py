...
def _load_pipeline(index_dir: str, model: str, top_k: int, use_reranker: bool) -> RAGPipeline:
    ...
    return RAGPipeline(retriever=retriever, claude_client=claude)
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        index_dir = st.text_input("Index directory", value=str(_DEFAULT_INDEX))

        st.divider()
        st.caption(f"Index: `{Path(index_dir).name}`")
        st.caption("Answers grounded in the GitLab handbook.")

    # ── Header ────────────────────────────────────────────────────────────────
    st.title("GitLab Handbook QA")
    st.caption("Ask anything about the GitLab handbook. Answers use only retrieved context.")
    ...
    if submit:
        ...
        pipeline = _load_pipeline(index_dir, model, top_k, use_reranker)
